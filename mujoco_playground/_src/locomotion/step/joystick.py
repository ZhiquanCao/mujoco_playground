...
# ==============================================================================
"""Joystick task for STEP Humanoid."""
...
from mujoco_playground._src.locomotion.step import base as step_base
from mujoco_playground._src.locomotion.step import step_constants as consts
  ...
  return config_dict.create(
      ...
      noise_config=config_dict.create(
          ...
          scales=config_dict.create(
              joint_pos=0.03, # Uniform noise scale for each joint
              joint_vel=1.5,
              gravity=0.05,
              linvel=0.1,
              gyro=0.2,
      ...
      # Used to sample commands during training.
...
class Joystick(step_base.StepEnv):
  ...
  def _post_init(self) -> None:
    ...
    self._default_pose = jp.array(
        self._mj_model.keyframe("home").qpos[7:]
    )
    ...
    self._weights = jp.array([
        0.01, 1.0, 1.0, 0.01, 1.0,  # left leg.
        0.01, 1.0, 1.0, 0.01, 1.0,  # right leg.
        # 1.0, 1.0, 1.0,  # waist.
    ...
    self._torso_imu_site_id = self._mj_model.site("imu").id
    ...
    self._torso_geom_id = self._mj_model.geom("torso").id
    print("Torso geom id: ", self._torso_geom_id)
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    rng, key = jax.random.split(rng)
    qpos = qpos.at[7:].set(
        qpos[7:] * jax.random.uniform(key, (consts.NUM_DOF,), minval=0.5, maxval=1.5)
    )
  ...
  def _get_obs(
    ...
    gravity = data.site_xmat[self._torso_imu_site_id].T @ jp.array([0, 0, -1])
    info["rng"], noise_rng = jax.random.split(info["rng"])
    noisy_gravity = (
        gravity
  ...
  def _reward_feet_air_time(
    ...
    reward *= cmd_norm > 0.1 # Only reward when moving.
    return reward
