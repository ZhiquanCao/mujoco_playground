...
NUM_DOF = consts.NUM_DOF
FLOOR_GEOM_ID = consts.FLOOR_GEOM_ID
TORSO_BODY_ID = consts.TORSO_BODY_ID
...
def domain_randomize(model: mjx.Model, rng: jax.Array):
  ...
  def rand_dynamics(rng):
    ...
    frictionloss = model.dof_frictionloss[6:] * jax.random.uniform(
        key, shape=(NUM_DOF,), minval=0.5, maxval=2.0
    )
    ...
    armature = model.dof_armature[6:] * jax.random.uniform(
        key, shape=(NUM_DOF,), minval=1.0, maxval=1.05
    )
    ...
    qpos0 = qpos0.at[7:].set(
        qpos0[7:]
        + jax.random.uniform(key, shape=(NUM_DOF,), minval=-0.05, maxval=0.05)
    )
